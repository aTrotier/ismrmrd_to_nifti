...
%images
[head, hdr, img_scaled] = load_ismrmrd(filename);




%% Display the first image (mid slice) : uncomment to display
% center=size(img_scaled{1})/2
% close all
% figure
% colormap gray
% subplot(3,1,1);
% imagesc(reconImages{1}(:,:,center(3))); axis image; axis off; colorbar;
% subplot(3,1,2);
% imagesc(squeeze(reconImages{1}(:,center(2),:))); axis image; axis off; colorbar;
% subplot(3,1,3);
% imagesc(squeeze(reconImages{1}(center(1),:,:))); axis image; axis off; colorbar;
% subplot(2,1,2);
...
h = create_nifti_parameters(head, hdr, table_position_offset);
...
% Plot the mid slice : uncomment to plot
% figure
% colormap gray
% subplot(2,1,1);
% imagesc(img(:,:,size(img,3)/2)); axis image; axis off; colorbar;
...
h2{1}=h;
[nii2, h3] = set_nii_hdr_manon(nii, h2, pf);
